...
export default {
    ...
    async execute({ sock, msg, args, phoneNumber, jid, isGroup }) {
        try {
            // Vérifier si c'est le propriétaire (sécurité supplémentaire)
            if (!isOwner(msg, phoneNumber)) {
            ...
            // Ajouter l'utilisateur comme sudo
            const success = sudoManager.addSudoUser(phoneNumber, targetUserJid);
            
            if (success) {
                const displayNumber = targetUserJid.split('@')[0];
                
                // Réponse avec image et caption
                await sock.sendMessage(jid, {
                    image: {
                        // Remplacez par l'URL de votre image ou le buffer
                        url: 'https://i.postimg.cc/tg8QrNNj/Uzui-Tengen-with-his-three-wives.jpg' // Image de succès
                    },
                    caption: formatSuccess(`> _*SUDO PRIVILEGES GRANTED*_\n\n👤 User: ${displayNumber}\n`),
                    mentions: [targetUserJid]
                }, { quoted: null });
                
                // Informer l'utilisateur s'il est dans le groupe
            } else {
                // Réponse d'erreur avec image
                await sock.sendMessage(jid, {
                    image: {
                        url: 'https://i.postimg.cc/tg8QrNNj/Uzui-Tengen-with-his-three-wives.jpg' // Image d'erreur
                    },
            ...
            console.error(`❌ [${phoneNumber}] Sudo command error:`, error);
            
            // Réponse d'erreur générale avec image
            await sock.sendMessage(jid, {
                image: {
                    url: 'https://i.postimg.cc/tg8QrNNj/Uzui-Tengen-with-his-three-wives.jpg' // Image d'erreur générale
                },
