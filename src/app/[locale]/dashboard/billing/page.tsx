...
import { Bot, CreditCard, Check, Zap, ArrowLeft, Sun, Moon, Smartphone, MessageSquare, Wallet, Crown, Infinity } from 'lucide-react';
  ...
  const handleBuy = async (pack: any) => {
    try {
        const res = await fetch('/api/billing/create', {
            ...
            body: JSON.stringify({ 
                packId: pack.isPremium ? 'premium' : pack.coins, 
                price: pack.price,
                coins: pack.coins
            })
        });
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 transition-colors duration-300">
          ...
          <div className="flex items-center gap-8">
            <Link href="/" className="text-xl font-bold tracking-tighter flex items-center gap-2">
             ...
             <div className="hidden md:flex items-center gap-6 text-sm font-medium text-neutral-500">
              ...
              <Link href="/dashboard/billing" className="text-neutral-900 dark:text-neutral-100">{d('nav.billing')}</Link>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2 bg-neutral-100 dark:bg-neutral-900 px-3 py-1.5 rounded-full border border-neutral-200 dark:border-neutral-800">
              {isPremium ? (
                  <span className="flex items-center gap-1 text-sm font-bold text-yellow-500">
                      <Crown className="w-4 h-4 fill-yellow-500" /> Premium
                  </span>
              ) : (
                  <>
      ...
      <main className="pt-24 pb-12 px-6 max-w-7xl mx-auto">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <p className="text-neutral-500 dark:text-neutral-400 text-lg">{t('subtitle')}</p>
        </div>
        ...
        <motion.div 
            ...
            className="mb-16 rounded-3xl bg-gradient-to-br from-indigo-900 via-purple-900 to-black text-white p-8 md:p-12 relative overflow-hidden shadow-2xl shadow-indigo-500/20"
        >
            <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500 rounded-full blur-[100px] opacity-30 -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex-1 text-center md:text-left">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-yellow-400 text-sm font-bold mb-6">
                        <Crown className="w-4 h-4 fill-yellow-400" />
                        {t('premium_banner.limited_offer')}
                    </div>
                    <h2 className="text-3xl md:text-5xl font-bold mb-4 tracking-tight">{t('premium_banner.title')}</h2>
                    <p className="text-indigo-200 text-lg mb-8 max-w-xl">
                        {t('premium_banner.description')}
                    </p>
                    <ul className="space-y-3 mb-8 text-left inline-block">
                        <li className="flex items-center gap-3">
                            ...
                            <span className="font-medium">{t('premium_banner.features.bots')}</span>
                        </li>
                        <li className="flex items-center gap-3">
                            <div className="p-1 rounded-full bg-green-500/20 text-green-400"><Check className="w-4 h-4" /></div>
                            <span className="font-medium">{t('premium_banner.features.coins')}</span>
                        </li>
                        <li className="flex items-center gap-3">
                            <div className="p-1 rounded-full bg-green-500/20 text-green-400"><Check className="w-4 h-4" /></div>
                            <span className="font-medium">{t('premium_banner.features.support')}</span>
                        </li>
                    </ul>
                    <div className="flex flex-col sm:flex-row gap-4">
                        <button 
                            onClick={() => handleBuy({ price: 15, coins: 0, isPremium: true })}
                            className="px-8 py-4 bg-white text-indigo-900 rounded-xl font-bold text-lg hover:bg-neutral-100 transition-colors shadow-lg shadow-white/10"
                        >
                ...
                <div className="hidden md:block">
                     <Infinity className="w-64 h-64 text-white/5" />
                </div>
            </div>
        </motion.div>

        <h2 className="text-2xl font-bold mb-8 text-center">{t('top_up_title')}</h2>
                ...
                >
                    ...
                    <ul className="space-y-4 mb-8 flex-1">
                        <li className="flex items-center gap-3 text-sm text-neutral-600 dark:text-neutral-400">
                            ...
                            {t('packs.features.instant')}
                        </li>
                        <li className="flex items-center gap-3 text-sm text-neutral-600 dark:text-neutral-400">
                            <Check className="w-5 h-5 text-green-500" />
                            {t('packs.features.priority')}
                        </li>
                         <li className="flex items-center gap-3 text-sm text-neutral-600 dark:text-neutral-400">
                            <Check className="w-5 h-5 text-green-500" />
                            {t('packs.features.lifetime')}
                        </li>
                    </ul>
        ...
        <div className="max-w-5xl mx-auto">
            <h2 className="text-2xl font-bold mb-8 text-center">{t('manual.title')}</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="p-6 rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 flex flex-col">
                    ...
                    <h3 className="text-lg font-bold mb-4">{t('manual.local.title')}</h3>
                    ...
                    <div className="mt-auto flex items-start gap-2 text-sm text-neutral-500">
                         ...
                         <span>{t('manual.local.instruction', { number: '+237 698 711 207' })}</span>
                    </div>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 flex flex-col">
                    ...
                    <h3 className="text-lg font-bold mb-2">{t('manual.minipay.title')}</h3>
                    <p className="text-sm text-neutral-500 mb-4">{t('manual.minipay.desc')}</p>
                    <ol className="text-sm space-y-3 mb-6 list-decimal pl-4 text-neutral-600 dark:text-neutral-400">
                        <li><a href="https://link.minipay.xyz/invite?ref=1KT7MleY" target="_blank" className="text-indigo-500 hover:underline">{t('manual.minipay.steps.0')}</a></li>
                        <li>{t('manual.minipay.steps.1')}</li>
                        <li>{t('manual.minipay.steps.2', { number: '650471093' })}</li>
                    </ol>
                     <div className="mt-auto pt-4 flex items-center gap-2 text-xs text-neutral-500">
                        <Check className="w-4 h-4 text-green-500" />
                        {t('manual.minipay.manual_check')}
                    </div>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 md:col-span-2 lg:col-span-1 flex flex-col">
                    ...
                    <h3 className="text-lg font-bold mb-4">{t('manual.binance.title')}</h3>
                    <div className="space-y-4 mb-6">
                        ...
                        <p className="text-sm text-center text-neutral-500">
                             {t('manual.binance.instruction')} <br/>
