...
import { Shield, Users, Database, Coins, Lock, Settings as SettingsIcon, Save, Activity, Trash2, FileCode } from 'lucide-react';
  ...
  const handleTogglePremium = async (userId: string, currentStatus: boolean) => {
    try {
        const res = await fetch('/api/admin/users/premium', {
            ...
            body: JSON.stringify({ userId, isPremium: !currentStatus, days: 30 })
        });
        if (res.ok) {
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-4 md:p-8">
        <header className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between mb-8 md:mb-12 gap-4">
            ...
            <div className="flex gap-4">
                <Link href="/steph/templates" className="px-4 py-2 bg-indigo-600 border border-indigo-500 rounded-lg text-sm hover:bg-indigo-700 transition-colors flex items-center gap-2">
        ...
        {/* Stats Grid */}
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12">
            <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-2xl relative overflow-hidden group">
             ...
             <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-2xl relative overflow-hidden group">
                ...
                <div className="flex items-center gap-4 mb-2 relative z-10">
                    ...
                    <span className="text-neutral-400">Total Bots</span>
                </div>
                <p className="text-3xl font-bold relative z-10">{stats?.totalBotsCount || 0}</p>
            </div>
             <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-2xl relative overflow-hidden group">
                ...
                <p className="text-3xl font-bold relative z-10">{stats?.activeBotsCount || 0}</p>
            </div>
             <div className="p-6 bg-neutral-900 border border-neutral-800 rounded-2xl relative overflow-hidden group">
                             ...
                             <tr key={u.id} className="border-b border-neutral-800/50 hover:bg-neutral-800/20 transition-colors">
                                 ...
                                 <td className="py-4">
                                     <button 
                                        ...
                                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-colors ${u.isPremium ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/20' : 'bg-neutral-800 text-neutral-500 border border-neutral-700'}`}
                                     >
                                 ...
                                 <td className="py-4">
                                     <div className="flex flex-wrap gap-1">
                                        <span className="px-2 py-1 bg-neutral-800 rounded text-xs font-bold">{u.bots.length}</span>
                                        {u.bots.map((b: any) => (
                                            <span key={b.id} className="px-2 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded text-[10px] uppercase font-bold">
                                                {b.template}
                                            </span>
                                        ))}
                                     </div>
